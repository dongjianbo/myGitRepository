	...
	<div>
		<c:choose>
			<c:when test="${d eq null}">
				<c:set var="form_action" value="${path }/manufer/insert.do"></c:set>
			</c:when>
			<c:otherwise>
				<c:set var="form_action" value="${path }/manufer/update.do"></c:set>
			</c:otherwise>
		</c:choose>
		<form action="${form_action}" method="post">
			...
			<ul>
				...
				<li>&nbsp; <c:if test="${d ne null}">
						<input type="hidden" name="idmanufer" value="${d.idmanufer }" />
					</c:if>
