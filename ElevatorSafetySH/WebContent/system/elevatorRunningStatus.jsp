...
<head>
	...
	<title>电梯实时运行状态</title>
	<link href="${path}/css/system.css" rel="stylesheet" type="text/css">
	<link href="${path}/css/table.css" rel="stylesheet" type="text/css">
	<link rel="stylesheet" href="${path}/jquery/themes/base/jquery.ui.all.css">
	<script type="text/javascript" src="${path}/jquery/jquery-1.10.2.js"></script>
 	<script type="text/javascript" src="${path}/jquery/ui/jquery.ui.core.js"></script>
 	<script type="text/javascript" src="${path}/jquery/ui/jquery.ui.widget.js"></script>
 	<script type="text/javascript" src="${path}/jquery/ui/jquery.ui.mouse.js"></script>
 	
	<script type="text/javascript" src="${path}/jquery/ui/jquery.ui.resizable.js"></script>
	<script type="text/javascript" src="${path}/js/pgHtml5Bridge.js"></script>
    <script type="text/javascript" src="${path}/js/pgLibExtVideo.js"></script>
    <script type="text/javascript" src="${path}/js/pgLibLiveMultiError.js"></script>
    <script type="text/javascript" src="${path}/js/pgLibLiveMultiRender.js"></script>
	...
	<style type="text/css">
		...
		.divstyle1{
			background-image: url('${path}/images/icon/floor.jpg');
    ...
    <script type="text/javascript">
    	$(function(){
    		pgInitialize();
    		pgCtrl();
    		setInterval(jsonRunningStatus, 1000);
    		$("#divVideo").resizable();
    	});
    	function jsonRunningStatus(){
    		$.getJSON("${path}/elevator/jsonRunningStatus.do","rand="+Math.random()+"&eid=${param.eid}",function(m){
    			//alert(JSON.stringify(m.history));
    			if(m.history!=null){
    				if(m.history.a!="FF"){
    					var history_a="";
            			switch(m.history.a){
            			case "00":
            				if(m.history.m=="1"){
            					history_a="<b style=\"color: red\">停梯后不开门</b>";
            				}
            				break;
            			case "01":history_a="<b style=\"color: red\">错位停梯</b>";break;
            			case "02":history_a="<b style=\"color: red\">错位停梯且开门</b>";break;
            			case "03":history_a="<b style=\"color: red\">错位停梯且困人</b>";break;
            			case "04":history_a="<b style=\"color: red\">错位停梯且开门，困人</b>";break;
            			case "05":history_a="<b style=\"color: red\">非平层停梯</b>";break;
            			case "06":history_a="<b style=\"color: red\">非平层停梯且开门</b>";break;
            			case "07":history_a="<b style=\"color: red\">非平层停梯且困人</b>";break;
            			case "08":history_a="<b style=\"color: red\">非平层停梯且开门，困人</b>";break;
            			case "09":history_a="<b style=\"color: red\">电梯运动中开门</b>";break;
            			case "0A":history_a="<b style=\"color: red\">电梯运动中开门且困人</b>";break;
            			}
            			$("#history_a").html("A类故障："+history_a);
            			if(m.history.a=="00"&&m.history.m=="0"){
            				$("#history_a").html("");
            			}
    				}else{
    					$("#history_a").html("");
    				...
    				if(m.history.b!="FF"){
    					var history_b="";
            			switch(m.history.b){
            			case "0B":history_b="<b style=\"color: red\">蹲底</b>";break;
            			case "0C":history_b="<b style=\"color: red\">冲顶</b>";break;
            			case "80":history_b="<b style=\"color: red\">系统外供电断开</b>";break;
            			case "8B":history_b="<b style=\"color: red\">系统外供电(应急照明外供电)断开-暨-蹲底</b>";break;
            			case "8C":history_b="<b style=\"color: red\">系统外供电(应急照明外供电)断开-暨-冲顶</b>";break;
            			}
            			$("#history_b").html("B类故障："+history_b);
    				}else{
    					$("#history_b").html("");
    				}
    				$("#history_f").html(m.history.f);
    				switch(m.history.u){
    				case "0F":
    					$("#history_u").attr("src","${path}/images/icon/up.jpg");
    					$("#history_u").attr("title","上行");
    					$("#history_u").attr("alt","上行");
    					break;
    				case "10":
    					$("#history_u").attr("src","${path}/images/icon/down.jpg");
    					$("#history_u").attr("title","下行");
    					$("#history_u").attr("alt","下行");
    					break;
    				case "FF":
    					$("#history_u").attr("src","${path}/images/icon/stop.jpg");
    					$("#history_u").attr("title","停梯");
    					$("#history_u").attr("alt","停梯");
    					break;
    				}
    				switch(m.history.d){
    				case "0D":
    					$("#history_d").attr("src","${path}/images/icon/open.jpg");
    					$("#history_d").attr("title","开门");
    					$("#history_d").attr("alt","开门");
    					break;
    				case "0E":
    					$("#history_d").attr("src","${path}/images/icon/close.jpg");
    					$("#history_d").attr("title","关门");
    					$("#history_d").attr("alt","关门");
    					break;
    				}
    				switch(m.history.m){
    				case "1":
    					$("#history_m").attr("src","${path}/images/icon/have.jpg");
    					$("#history_m").attr("title","有人");
    					$("#history_m").attr("alt","有人");
    					break;
    				case "0":
    					$("#history_m").attr("src","${path}/images/icon/none.jpg");
    					$("#history_m").attr("title","无人");
    					$("#history_m").attr("alt","无人");
    					break;
    				}
    			}
...
<body >
	<h4>电梯：<b>${e.desc}</b></h4>
	<h4>地址：<b>${e.address}</b></h4>
		<button class="closeButton" onclick="javascript:history.back();">退出本页面</button>
	<p>
	<c:if test="${history eq null}">
		<p>暂无数据！</p>
	</c:if>
	<c:if test="${history.a ne 'FF'}">
		<b id="history_a">
		A类故障：
		<c:choose>
			<c:when test="${history.a eq '00' and history.m ne '0'}">
				<b style="color: red">停梯后不开门</b>
			</c:when>
			<c:when test="${history.a eq '01'}">
				<b style="color: red">错位停梯</b>
			</c:when>
			<c:when test="${history.a eq '02'}">
				<b style="color: red">错位停梯且开门</b>
			</c:when>
			<c:when test="${history.a eq '03'}">
				<b style="color: red">错位停梯且困人</b>
			</c:when>
			<c:when test="${history.a eq '04'}">
				<b style="color: red">错位停梯且开门，困人</b>
			</c:when>
			<c:when test="${history.a eq '05'}">
				<b style="color: red">非平层停梯</b>
			</c:when>
			<c:when test="${history.a eq '06'}">
				<b style="color: red">非平层停梯且开门</b>
			</c:when>
			<c:when test="${history.a eq '07'}">
				<b style="color: red">非平层停梯且困人</b>
			</c:when>
			<c:when test="${history.a eq '08'}">
				<b style="color: red">非平层停梯且开门，困人</b>
			</c:when>
			<c:when test="${history.a eq '09'}">
				<b style="color: red">电梯运动中开门</b>
			</c:when>
			<c:when test="${history.a eq '0A'}">
				<b style="color: red">电梯运动中开门且困人</b>
			</c:when>
		</c:choose>
		</b>
	</c:if>
	&nbsp;&nbsp;&nbsp;&nbsp;
	<c:if test="${history.b ne 'FF'}">
		<b id="history_b">
		B类故障：
		<c:choose>
			<c:when test="${history.b eq '0B'}">
				<b style="color: red">蹲底</b>
			</c:when>
			<c:when test="${history.a eq '0C'}">
				<b style="color: red">冲顶</b>
			</c:when>
			<c:when test="${history.a eq '80'}">
				<b style="color: red">系统外供电断开</b>
			</c:when>
			<c:when test="${history.a eq '8B'}">
				<b style="color: red">系统外供电(应急照明外供电)断开-暨-蹲底</b>
			</c:when>
			<c:when test="${history.a eq '8C'}">
				<b style="color: red">系统外供电(应急照明外供电)断开-暨-冲顶</b>
			</c:when>
	...
	<div style="position:absolute;margin-left: 550px;margin-top:90px;">
		<ul>
			
			<li style="list-style-type: none">
			<div class="divstyle1">
				<b id="history_f" style="color:blue">${history.f}</b>
			</div>
			<li style="list-style-type: none">
			<c:choose>
			<c:when test="${history.u eq '0F'}">
				<img id="history_u" alt="上行" title="上行" src="${path}/images/icon/up.jpg">
			</c:when>
			<c:when test="${history.u eq '10'}">
				<img id="history_u" alt="下行" title="下行" src="${path}/images/icon/down.jpg">
			</c:when>
			<c:when test="${history.u eq 'FF'}">
				<img id="history_u" alt="停梯" title="停梯" src="${path}/images/icon/stop.jpg">
			</c:when>
			</c:choose>
			<li style="list-style-type: none">
			<c:choose>
			<c:when test="${history.d eq '0D'}">
				<img id="history_d" alt="开门" title="开门" src="${path}/images/icon/open.jpg">
			</c:when>
			<c:when test="${history.d eq '0E'}">
				<img id="history_d" alt="关门" title="关门" src="${path}/images/icon/close.jpg">
			</c:when>
			</c:choose>
			<li style="list-style-type: none">
			<c:choose>
			<c:when test="${history.m eq '1'}">
				<img id="history_m" alt="有人" title="有人" src="${path}/images/icon/have.jpg">
			</c:when>
			<c:when test="${history.m eq '0'}">
				<img id="history_m" alt="无人" title="无人" src="${path}/images/icon/none.jpg">
			</c:when>
			</c:choose>
		</ul>
	</div>
	<hr>
	<div id="divVideo" class="ui-widget-content" style="border:1px solid #cccccc;background:#000; color:#FFF; width:540px; height:380px"></div>
	<span id="id_LiveSta"></span>
	<div style="display:none">
	<p>故障说明：</p>
	<pre style="color:red">
	A 类故障 ： 
	00, //平层后一直不开门，即判定为停梯后打不开门 
	01, //错位停梯 
	02, //错位停梯 且 开门 
	03, //错位停梯 且 困人 
	04, //错位停梯 且 开门 困人 
	...
	0C, //冲顶 
	80, //系统外供电断开 
	8B, //系统外供电(应急照明外供电)断开 -暨- 蹲底 
	8C, //系统外供电(应急照明外供电)断开 -暨- 冲顶
	FF, //无 B 类故障，电梯正常
	</pre>
	</div>
	<table width="640" style="display: none;" border="0">
        ...
        <tr>
            <td style="padding:3px">
                ...
                <span>设备ID: </span>
                <input id="id_CaptureID" type="text" value="${history.e_no}" />
    ...
    <table width="646" style="display:none" border="1" cellpadding="0" cellspacing="0" bordercolor="#93A9E3">
        <tr>
	...
	<script type="text/javascript">
        ...
        function FileAccept(sPeer) {
            ...
            bIsReplying = false;
            bTransfering = true;
            findByID("info").innerHTML = sPeer + "接受了文件传输:..." + "<a href='#' onclick='btnRefuse()'>取消</a>";
        }
        ...
        function FileReject(sPeer) {
            ...
            bIsReplying = false;
            bTransfering = false;
            findByID("info").innerHTML = sPeer + "拒绝了文件传输:...";
        }
        ...
        var pgLiveUI = {
            OnEvent: function(sAct, sData, sCapID) {
                ...
                if (sAct == "VideoStatus") {
                    ...
                    findByID("id_LiveSta").innerText = "速率: " + sBitRate + "bps，帧速: " +
        ...
        function FileFinish(sPeer) {
            ...
            bIsReplying = false;
            bTransfering = false;
            findByID("info").innerHTML = "与" + sPeer + "端文件传输完毕...";
        }
        ...
        function FileAbort(sData) {
            ...
            bTransfering = false;
            findByID("info").innerHTML = "文件传输中断了，文件信息:本地文件路径:" + sPath + " 已传输大小:" + sCurSize +
                "/文件总大小:" + sReqSize;
        }
        ...
        function btnPut() {
            ...
            if (iErr == -1) {
                ...
                bTransfering = true;
                bNeedCallback = false;
                findByID("info").innerHTML = "您正在请求文件上传..." + "<a href='#' onclick='btnRefuse()'>取消</a>";
            } else {
                findByID("info").innerHTML = "文件上传请求失败(错误码:" + iErr + ")...";
            }
        }
        ...
        function btnGet() {
            ...
            if (iErr == -1) {
                ...
                bTransfering = true;
                bNeedCallback = false;
                findByID("info").innerHTML = "您正在请求文件下载..." + "<a href='#' onclick='btnRefuse()'>取消</a>";
            } else {
                findByID("info").innerHTML = "文件下载请求失败(错误码:" + iErr + ")...";
            }
        }
        ...
        function FileProgress(sData) {
            ...
            var sReqSize = getcontent(sData, "total");
            var sCurSize = getcontent(sData, "position");
            findByID("FileProgress").innerHTML = "本地文件路径:" + sPath + " 已传输大小:" + sCurSize +
                "/文件总大小:" + sReqSize;
        }
        ...
        function FilePutRequest(sData, sPeer) {
            ...
            bIsReplying = true;
            bNeedCallback = true;
            findByID("info").innerHTML = "是否接受" + sPeer + "的文件(" + file + ")上传请求:<a href='#' onclick='btnAccept()'>接受</a>or<a href='#' onclick='btnRefuse()'>拒绝</a>";
            iMethID = 32;
        }
        ...
        function FileGetRequest(sData, sPeer) {
            ...
            bIsReplying = true;
            sReplysFile = file;
            findByID("info").innerHTML = "是否接受" + sPeer + "的文件(" + file + ")下载请求:<a href='#' onclick='btnAccept()'>接受</a>or<a href='#' onclick='btnRefuse()'>拒绝</a>";
            iMethID = 33;
        }
        ...
        function dearFileRequest(sPeer, sPeerPath, iAction) {
            ...
            if (iAction != 1) {
                ...
                bIsZhuDong = false;
                bTransfering = true;
                findByID("info").innerHTML = "您正在进行文件(" + sPeerPath + ")传输:...<a href='#' onclick='btnRefuse()'>取消</a>";
            } else {
                bTransfering = false;
                findByID("info").innerHTML = "您拒绝了文件(" + sPeerPath + ")传输:...";
            }
        }
